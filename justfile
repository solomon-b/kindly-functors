...
_check-version:
    #!/usr/bin/env bash
    set -euo pipefail
    cabal_v=$(awk '/^version:/{print $2; exit}' kindly-functors.cabal)
...
release VERSION: _require-clean-main
    ...
    sed -i -E "s/^(version:[[:space:]]*).*/\1${version}/" kindly-functors.cabal
...
publish-candidate: _require-clean-main _check-version
    ...
    cabal upload "$sdist"
    echo "Uploaded candidate. Review: https://hackage.haskell.org/package/kindly-functors-${version}/candidate"
...
publish: _require-clean-main _check-version _require-unreleased test
    ...
    echo "Published kindly-functors-${version} to Hackage and pushed tag ${version}."
